#!/usr/bin/env python3
"""MARKET AI ENGINE 6.0 — LEADER PROPAGATION ENGINE (o líder já se moveu → entrar nos atrasados com edge comprovado) sobre o núcleo 5.0
(informação explícita + implícita · reação temporal · propagação entre ativos).

Gerado por tools/build_single_file.py a partir do pacote gold_ai/ (versão 6.0.0).
Equivalente a `python -m gold_ai`. Único bundle suportado (v1–v5 removidos). 6.0 = núcleo 5.0 + LEADER PROPAGATION ENGINE.

6.0 — LEADER PROPAGATION ENGINE: o robô não prevê o primeiro movimento. Detecta que ele JÁ começou no líder (impulso ≥ zσ em segundos/M1),
consulta o LAG MAP (líder × direção → alvo: P da direção, tempo de reação, fração transmitida) e entra SÓ nos atrasados com edge líquido
comprovado por operações-sombra; alvo adaptativo = fração histórica × impulso do líder; sai. TESTES A→E (cada ativo sozinho × líder → 1..4 atrasados):
...
__version__ = "6.0.0"
__build__ = "2026-09-17 18:35 UTC · 5904ecf+"
...
propagation_session = session_of
